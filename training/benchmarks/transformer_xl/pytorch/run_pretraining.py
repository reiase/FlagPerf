...
def main():
    ...
    with event_guard(instance_driver, Event.INIT_START, Event.INIT_END):
        ...
        dist_pytorch.barrier(config.vendor)
        trainer.init(config, train_dataset)

        dist_pytorch.barrier(config.vendor)
        init_evaluation_start = time.time()
        eval_loss = None  # evaluator.evaluate(trainer)
        training_state.eval_loss = eval_loss
        init_evaluation_end = time.time()
        init_evaluation_info = dict(
            eval_loss=eval_loss, time=init_evaluation_end - init_evaluation_start
        )
...
if __name__ == "__main__":
    ...
    if config.do_train:
        finished_info = {
            ...
            "throughput(sps)_raw": trained_samples / state.raw_train_time,
            "throughput(sps)_no_eval": trained_samples / state.no_eval_time,
            "throughput(sps)_pure_compute": trained_samples / state.pure_compute_time,
